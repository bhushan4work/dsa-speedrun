...
int getLongestSubarray(vector<int>& arr, int k){
    //(brute)- t.c.- O(n^2)  s.c- O(1) for both (only +ves & -ves +ves arr) but may give a tle error
    int length = 0;
    ...
    // (hashing) is better soln for arr(having +ves) & optimal soln for arr(having +ves & -ves) t.c -O(nlogn) s.c. -O(n)
    ...
    //(optimal) 2 pointer - for arr(having +ves) t.c- O(2n)  s.c- O(1)
