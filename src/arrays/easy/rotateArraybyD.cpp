...
vector<int> rotateArray(vector<int>arr, int k) {
    ...
    //method1(brute force) t.c.- O(k+ (n-k) + k ) = O(n+k) , s.c.-(extra space)- O(k)
    //for left rotation - (for codeStudio que)
    int temp[n]; //or vector<int> temp(k);
    for(int i=0;i<k;i++){ 
        temp[i] = arr[i]; //store the elements that is to be rotated in an temp arr
    }
    for(int i=k;i<n;i++){
    ...
    //for left rotation - (for codeStudio que)
    reverse(arr.begin(),arr.begin()+k); //last element i.e arr+k is exclusive(or <arr+k) in reverse fxn
