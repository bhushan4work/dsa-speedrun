...
vector<int> getSecondOrderElements(int n, vector<int> a) {
    ...
    //2nd largest t.c.- O(n)
    int largest = a[0];
    int slargest = -1; //take a smallest value possible or INT_MIN
    for(int i=0;i<n;i++){
        if(a[i] > largest){
            slargest = largest; //if bigger val than largest val exists then it changes & prev largest val is given to slargest
            largest = a[i];
        }
    ...
    int smallest = a[0];
    int ssmallest = INT_MAX; //take a greatest value possible
    for(int i=0;i<n;i++){
        if(a[i] < smallest){
            ssmallest = smallest;
            smallest = a[i];
