...
//FAQ- rotate n*n 2D matrix by 90 deg anti-clockwise
//for anti-cloc we reverse it first n then transpose
...
void rotateClockwise(vector<vector<int>> &matrix){
    ...
    //Transpose the matrix - (diagonal elements remain same while we transpose)
    for (int i = 0; i < n; ++i){  //n-1 is done as we need to swap elements above diagonal only
        for (int j = i+1; j < n; ++j){
            // Swap element at (i, j) with (j, i) to transpose
